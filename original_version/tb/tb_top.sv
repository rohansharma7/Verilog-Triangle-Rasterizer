...
// triangle landed in screen_mem. doesn't check display_interface's SPI output,
// tb_display_interface does that
module tb_top;
    logic clk = 0;
    logic rst_n;
    ...
    task automatic run_fake_touch();
        ...
        if (cmd_byte == 8'hD0)
            drive_result(fake_x);
        else if (cmd_byte == 8'h90)
            drive_result(fake_y);
        else
            drive_result(12'h000);
    ...
    // peek at mem[] directly instead of driving rd_addr - display_interface
